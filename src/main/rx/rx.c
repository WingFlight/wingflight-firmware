...
void rxFrameCheck(timeUs_t currentTimeUs, timeDelta_t currentDeltaTimeUs)
{
    ...
    // once every 100ms against a backup stream arriving every ~20ms: besides making
    ...
    // read provider-owned frame state while the receive ISR was mid-write on a newer frame -
    // a torn read mixing old and new bits across adjacent channel fields, i.e. exactly the
...
void detectAndApplySignalLossBehaviour(void)
{
    ...
    rxInputBackupPoll();
    // Cheap early-out when idle - see rxInputBackupTrialTick().
    rxInputBackupTrialTick();
    ...
    if (!rxSignalReceived && rxInputBackupIsActive()) {
        for (int channel = 0; channel < activeRcChannelCount; channel++) {
            ...
            const uint8_t rawChannel = channel < RX_MAPPABLE_CHANNEL_COUNT ? rxConfig()->rcmap[channel] : channel;
            // Channels the backup doesn't carry (e.g. CH19-24 from a 24-channel
            // main RX) hold their last value instead of dropping to minimum.
            if (rawChannel < rxInputBackupGetChannelCount()) {
