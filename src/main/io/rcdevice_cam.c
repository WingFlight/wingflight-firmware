...
static void rcdevice5KeySimulationProcess(timeUs_t currentTimeUs)
{
    ...
    if (isButtonPressed) {
        ...
        if (IS_MID(ROLL) && IS_MID(PITCH) && IS_LO(YAW)) { // Disconnect Lo YAW
            if (rcdeviceInMenu) {
            ...
            if (rcdeviceInMenu) {
                ...
                } else if (IS_MID(ROLL) && IS_MID(PITCH) && IS_HI(YAW)) { // Enter HI YAW
                    key = RCDEVICE_CAM_KEY_ENTER;
                }
            } else {
                if (IS_MID(ROLL) && IS_MID(PITCH) && IS_HI(YAW)) { // Enter HI YAW
